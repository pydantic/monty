...
session semantics are identical to the native pool; the divergences below are
specific to this transport.

- **Async-only host calls.** A worker answers external functions, `os`
  callbacks, name lookups, and futures over `postMessage`, which is
  asynchronous. Synchronous host calls from inside the sandbox (which would
  need `SharedArrayBuffer` + `Atomics.wait`, and thus cross-origin isolation)
  are not supported.
- **No filesystem mounts.** There is no host filesystem in a browser worker, so
  `feed` rejects any mounts (`the wasm worker does not support filesystem
  mounts`). OS calls bubble to the `os` callback exactly as in the native path.
- **`print` is buffered per turn**, then flushed through the print callback
  before the turn resolves, rather than streamed incrementally as on the native
  pipe. Ordering within a turn is preserved.
- **Error tracebacks are not yet rendered.** Error turns carry the exception
  type, message, and structured frames; the worker-rendered traceback string is
  empty pending the generated codec.
- **Value marshalling is in TypeScript** (the native path converts in Rust via
  napi). The representation matches `convert.rs` for scalars, BigInt, `bytes`
  (`Buffer`), lists, tuples (`__tuple__` marker), named tuples (→ tuple), dicts
  (`Map`), sets, the datetime family, dataclasses, function values (→ name),
  and the marker types. **File handles** are not ported — `convert.rs` renders
  them as a Rust-side `repr` string this layer cannot reproduce — and raise a
  clear error rather than mis-converting.
- **In-process degrade.** Where `Worker` is unavailable, `createWorkerPool`
  runs the wasm on the calling thread with the same API — but with no crash
  isolation (a hard wasm abort takes the page) and no preemption (a runaway
  turn cannot be interrupted; only `request_timeout` over a real `Worker` can
  hard-kill it).
- **Turnkey loading is environment-specific.** `createMonty()` auto-loads the
  bundled module (Node reads it from disk, browsers `fetch` + compile it via
  the `node`/`browser` export conditions); bundlers/runtimes those loaders do
  not cover must compile the module themselves and call `createWorkerPool`.

The legacy in-process napi API (`Monty`/`MontySnapshot`/`MontyRepl`/
`runMontyAsync`, the old `wasm32-wasip1-threads` napi surface) has been removed;
the `@pydantic/monty/wasm` subpath is now this Web Worker pool.
