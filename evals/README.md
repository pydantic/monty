...
The workloads come from agent-code practice and its literature: Anthropic's programmatic tool calling, CodeAct,
Cloudflare's Code Mode, smolagents and the Recursive Language Models paper.
...
| `agent_of_agents`  | Coordinate three research sub-agents from code and merge their findings.                                               | An agent with its own tools behind a host function, called in loops and gathered.                               |
...
| `anomaly_scan`     | Flag anomalous days in a year of hourly metrics.                                                                       | Moving averages, z-scores, `erf` for p-values over thousands of points.                                         |
...
| `polars_proxy`     | Analyse a DataFrame through a host-proxied polars API.                                                                 | `ClassInstance` / `ClassType` policies, method chaining, expressions built from host objects.                   |
| `sql_bridge`       | Answer a question no single SQL query can, by running queries from code.                                               | A host function running SQL, rows joined and aggregated in Python.                                              |
| `bulk_classify`    | Label 5,000 reviews with a model-backed host function.                                                                 | Several items per prompt, bounded `gather` fan-out, sub-call cost accounting.                                   |
...
| `expense_policy`   | Apply a written expense policy to a month of claims with a reason per decision.                                        | Rules as functions, `@dataclass` records, explanations attached to results.                                     |
...
| `redact_pii`       | Extract fields from emails and redact personal data.                                                                   | `re` with named groups, structured output, non-ASCII text.                                                      |
...
| `cohort_pivot`     | Monthly retention cohorts and a pivot from raw events.                                                                 | Nested grouping, window-style calculations over large lists.                                                    |
| `tz_meeting`       | Find a meeting slot across five regions on a DST changeover week.                                                      | `zoneinfo` or host-supplied offsets, `datetime` arithmetic.                                                     |
| `approval_gate`    | Pause a refund run for a human decision mid-flight, then resume.                                                       | Suspension at a host call via `feed_start`, resuming with the decision.                                         |
| `resume_batch`     | A long batch job that survives a worker restart.                                                                       | `dump()` mid-feed and `load_snapshot` on another worker.                                                        |
| `sliding_window`   | Stream events in pages and report per-minute windows.                                                                  | Pagination, windowed aggregates, bounded memory.                                                                |
...
| `plugin_rules`     | Run user-submitted validation plugins over a batch of records.                                                         | Functions and classes defined in the sandbox, plugin exceptions reported per record.                            |
...
| `progress_bar`     | Report progress from a run that takes a minute.                                                                        | `print` streamed to the host while the code runs.                                                               |
| `host_objects`     | Drive a host object's methods and read its attributes.                                                                 | Sync host functions, method calls on a `ClassInstance`.                                                         |
| `unicode_names`    | Normalise and dedupe customer names across scripts and accents.                                                        | `unicodedata` normalisation, `casefold`, non-ASCII `str` methods.                                               |
