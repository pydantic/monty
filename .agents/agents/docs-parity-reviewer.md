...
2. **`docs/` does not duplicate `limitations/`.** `docs/` states the shape (which modules
...
4. **Named duplication points agree.** These are stated in several places on purpose;
...
5. **Snippets run.** Run `make test-docs`. Every Python block in `docs/`, `README.md` and
...
6. **Every claim is traceable.** For each factual claim the change adds — a default, a
...
7. **New pages are in the nav.** A file under `docs/` that `mkdocs.yml`'s `nav:` does not
   list will not appear on the site.

8. **Security framing is preserved.** Where the change touches mounts, host callbacks,
