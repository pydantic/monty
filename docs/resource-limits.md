...
- **Compilation time.** Parsing and bytecode compilation happen before the VM exists and are not charged to the duration
    ...
    Source compiled at runtime by `eval()` / `exec()` is charged against the duration budget, and stays allocated for
    the rest of the session; see [eval_exec.md](limitations/eval_exec.md).
