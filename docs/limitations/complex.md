...
- **`hash(z)`** differs from CPython's value (see [builtins.md](builtins.md)), while still agreeing with the hash of an
    equal `int` or `float`, so mixed-type dict keys behave.
