...
- **A `partial` or cached function crossing the host boundary marshals as its `repr`.** Neither side can call back
    ...
    Both type objects are outside the host allowlist, so they cross as a proxy naming the type
    (see [host-values.md](host-values.md)) rather than as `functools.partial` and `functools._lru_cache_wrapper`.
