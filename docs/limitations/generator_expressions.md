...
- **Advancement cannot schedule async tasks or wait on pending futures.** Calling `asyncio.run()` from a lazy
    body raises `NotImplementedError` if it needs to schedule a coroutine through `asyncio.gather()` or await a
    pending external future.
    Coroutines that complete without suspension, empty gathers, and already-resolved futures can still run.
