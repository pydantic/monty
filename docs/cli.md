...
`date.today()` and `datetime.now()` read the machine's clock in UTC; `astimezone()` and `time.tzname` also report that zone; `time.time()` reads the machine's clock as Unix epoch seconds.
