...
<div class="rust-decl-links" data-links="HostClock ../monty-types/#hostclock" hidden></div>

Chooses what `date.today()` and `datetime.now()` read, replacing the
[`System`](monty-types.md#hostclock) clock a runner starts with.

Only [`run`](#montyrun) and [`run_no_limits`](#montyrun)
consult it: they have no host to suspend to. Under
[`start`](#montyrun) the host answers both calls itself, so a clock
set here is ignored.

[`Denied`](monty-types.md#hostclock) takes the clock away; [`Fixed`](monty-types.md#hostclock)
...
Chooses what `date.today()` and `datetime.now()` read, replacing the
[`System`](monty-types.md#hostclock) clock a session starts with.

Only the non-suspending [`feed_run`](#montyrepl) and
[`call_function`](#montyrepl) consult it. Under
[`feed_start`](#montyrepl) the host answers both calls itself, so
