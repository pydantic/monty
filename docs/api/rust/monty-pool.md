...
pub struct PoolConfig {
    ...
    pub feed_duration_limit_grace: Option<std::time::Duration>,
    /// `feed_duration_limit_grace` for the
    ...
    pub turn_duration_limit_grace: Option<std::time::Duration>,
...
[`Checkout::session_id`](#checkout), from the state last stored under that ID.
...
<div class="rust-decl-links" data-links="NamedValues ../monty-types/#namedvalues;MountSpec #mountspec;OnPrint #onprint;TurnEvent #turnevent;PoolError #poolerror" hidden></div>
...
sent, as a session-preserving [`PoolError::Runtime`](#poolerror)). The session's
...
(`os.chdir` included) unless [`Checkout::feed_with_cwd`](#checkout) switches it.
...
[`Checkout::feed`](#checkout) with an explicit switch of the sandbox working
...
[`PoolError::Runtime`](#poolerror) (`ValueError`), raised before any frame is sent.
...
The opaque session ID a storing relay minted, which [`Checkout::restore`](#checkout)
...
[`PoolError::Finished`](#poolerror); a raw driver is never auto-resumed.
...
pub struct ReplConfig {
    ...
    pub os_policy: monty_types::OsPolicy,
    /// Whether a serving relay may store the session; subprocess workers ignore it.
    pub persistence: Persistence,
}
```

<div class="rust-decl-links" data-links="monty_types::ResourceLimits ../monty-types/#resourcelimits;monty_types::TypeCheckingConfig ../monty-types/#typecheckingconfig;monty_types::AssertMessageAnnotations ../monty-types/#assertmessageannotations;monty_types::OsPolicy ../monty-types/#ospolicy;Persistence #persistence" hidden></div>
...
pub enum TurnEvent {
    ...
    FunctionCall { function_name: String, args: monty_types::CallArgs, call_id: u32, object_id: Option<monty_types::MontyUuid>, allow_eager_await: bool, position: monty_types::SourceRange },
    ...
    /// no-handler default.
    OsCall { function_name: String, args: monty_types::CallArgs, call_id: u32, allow_eager_await: bool, system_sleep: Option<std::time::Duration>, position: monty_types::SourceRange },
    ...
    NameLookup { name: String, object_id: Option<monty_types::MontyUuid>, position: monty_types::SourceRange },
    /// Every sandbox task is blocked on external futures — answer with
    /// `Checkout::resume_futures`.
    ResolveFutures { pending_call_ids: Vec<u32>, position: monty_types::SourceRange },
...
<div class="rust-decl-links" data-links="monty_types::CallArgs ../monty-types/#callargs;monty_types::MontyUuid ../monty-types/#montyuuid;monty_types::SourceRange ../monty-types/#sourcerange;monty_types::MontyObject ../monty-types/#montyobject" hidden></div>
...
A storing relay mints a session ID ([`Checkout::session_id`](#checkout)) that resumes the
session from any process. Local subprocess workers never store anything.

Implements: `Clone`, `Copy`, `Debug`, `Default`, `Eq`, `From<Persistence>`, `PartialEq`, `StructuralPartialEq`.

## DEFAULT_DURATION_LIMIT_GRACE

```rust
pub const DEFAULT_DURATION_LIMIT_GRACE: std::time::Duration = _;
```
