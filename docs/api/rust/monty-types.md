...
pub enum HostClock {
    ...
    /// `Denied` rather than failing some other way.
    Fixed { unix_seconds: i64, microsecond: u32, local_offset_seconds: i32 },
}
```
...
[`OsFunctionCall`](#osfunctioncall) to.
...
[`System`](#hostclock) (see `monty`'s `default_clock`), which a type-level
[`Denied`](#hostclock) would quietly contradict.
...
<div class="rust-decl-links" data-links="OsFunctionCall #osfunctioncall;MontyObject #montyobject" hidden></div>
...
is [`Denied`](#hostclock), the call is not a clock call, or the
...
<div class="rust-decl-links" data-links="MontyClassType #montyclasstype;MontyUuid #montyuuid;DictPairs #dictpairs" hidden></div>

A class instance crossing the sandbox boundary — the payload of
[`MontyObject::ClassInstance`](#montyobject).
...
<div class="rust-decl-links" data-links="MontyUuid #montyuuid;DictPairs #dictpairs" hidden></div>

A non-builtin class type object crossing the sandbox boundary — the
payload of [`MontyType::Instance`](#montytype) and the class half of
[`MontyClassInstance`](#montyclassinstance).
...
[`MontyObject`](#montyobject)s and would otherwise be uncharged). Boxed payloads
...
<div class="rust-decl-links" data-links="MontyClassType #montyclasstype;ExcType #exctype" hidden></div>
...
[`MontyType::Instance`](#montytype) carries the resolved [`MontyClassType`](#montyclasstype) (name, uuid,
flags), so a [`MontyType`](#montytype) is always self-contained: it can be serialized,
...
an [`InvalidInputError`](#invalidinputerror). Host class types round-trip.
...
Derefs to `&str` for routing; [`ToMontyObject`](#tomontyobject)
projects it back to [`MontyObject::Path`](#montyobject) at the host boundary.
...
<div class="rust-decl-links" data-links="MontyPath #montypath;PathStringDataArgs #pathstringdataargs;PathBytesDataArgs #pathbytesdataargs;OpenCallArgs #opencallargs;MkdirCallArgs #mkdircallargs;RenameCallArgs #renamecallargs;GetenvArgs #getenvargs;MontyTimeZone #montytimezone;UrandomArgs #urandomargs" hidden></div>
...
hosts constructing calls must use [`Self::check_path_null_bytes`](#osfunctioncall) first.
