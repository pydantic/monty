...
```rust
pub const MONTY_VERSION: &str = "1.0.0";
```
...
arming point, plus structures it keeps for life (see [`allocate_into_baseline`](#allocate_into_baseline)).
...
## allocate_into_baseline

```rust
pub fn allocate_into_baseline<T>(build: impl FnOnce() -> T) -> T;
```

Runs `build` and adds what it leaves allocated to [`BASELINE_MEMORY`](#baseline_memory), so a
structure the worker keeps for life (the type checker) is not charged to the
session that happened to build it first.

Only accurate while no other thread changes [`LIVE_MEMORY`](#live_memory). An unset baseline is
...
scan; the default for [`CompileOptions::source_scan_threshold`](#compileoptions).
