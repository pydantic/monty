...
Entropy has no in-process fallback: under `run`, an unseeded `random` draw or `os.urandom()` raises
`NotImplementedError`, while under `start` it pauses on an `os.urandom` call for the host to answer
