...
// === Exception-handler matching resource-limit tests ===

/// Test that matching against a deeply-nested tuple of exception types is
/// bounded by the recursion-depth limit.
///
/// The exception type passed to `except` is constructed at runtime, so the
/// tuple may be nested far deeper than the source-code nesting limit allows.
/// Without the depth bound the matcher would recurse on the host's native
/// stack inside a single bytecode instruction.
#[test]
#[cfg_attr(
    feature = "memory-model-checks",
    ignore = "resource exhaustion doesn't guarantee heap state consistency"
)]
fn except_nested_tuple_recursion_limit() {
    let code = r"
t = (ValueError,)
for _ in range(100):
    t = (t,)
...
";
    ...
    assert!(result.is_err(), "deep tuple matching should hit the recursion limit");
    let exc = result.unwrap_err();
    assert_eq!(exc.exc_type(), ExcType::RecursionError);
}

/// Test that shallow tuple-of-types matching still works under a recursion limit.
#[test]
fn except_shallow_tuple_within_recursion_limit() {
    let code = r"
ok = False
try:
    raise TypeError()
...
";
    ...
    assert!(result.is_ok(), "shallow tuple matching should succeed: {result:?}");
    assert_eq!(result.unwrap(), MontyObject::Bool(true));
}
