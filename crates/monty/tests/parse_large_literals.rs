...
fn large_float_literal_accepted() {
    ...
    let mut run = MontyRun::new(code.to_string(), "test.py", vec![], CompileOptions::default())
...
fn monty_object_repr_or_error_success() {
    // Returning a range produces a `Repr` node with the correct repr string.
    // This exercises the repr_or_error success path in MontyObject::export.
...
fn monty_object_repr_or_error_slice() {
    // Returning a slice produces a `Repr` node with the correct repr string.
...
fn monty_object_repr_or_error_dict_keys() {
    // Returning a dict_keys view produces a `Repr` node.
...
fn monty_object_repr_or_error_with_huge_int() {
    ...
    // in repr_or_error. The `Repr` node should contain the error message
