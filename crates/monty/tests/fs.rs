...
//!
//! Tests `MountTable::handle_os_call()` across all four mount modes (ReadWrite,
//! ReadOnly, OverlayMemory, OverlayDirectory) and all supported filesystem
...
// =============================================================================
// OverlayDirectory mode
// =============================================================================

#[test]
fn ovl_dir_reads_fall_through() {
    let dir = create_test_dir();
...
#[test]
fn ovl_dir_write_goes_to_upper() {
    let dir = create_test_dir();
    let upper = TempDir::new().unwrap();
    let mut mt = mount_at_mnt(
        &dir,
    ...
    call_write(
        ...
        OsFunction::WriteText,
        "/mnt/upper_file.txt",
        MontyObject::String("in upper".to_owned()),
    )
    .unwrap()
    .unwrap();
    ...
    // Readable via mount.
    assert_eq!(
        call_ok(&mut mt, OsFunction::ReadText, "/mnt/upper_file.txt"),
        MontyObject::String("in upper".to_owned())
    );
    // File exists in upper dir on host.
    assert_eq!(
        fs::read_to_string(upper.path().join("upper_file.txt")).unwrap(),
        "in upper"
    );
    // Lower dir unchanged.
    assert!(!dir.path().join("upper_file.txt").exists());
}

#[test]
fn ovl_dir_whiteout_on_unlink() {
    let dir = create_test_dir();
    let upper = TempDir::new().unwrap();
    let mut mt = mount_at_mnt(
        &dir,
    ...
    // Whiteout file exists.
    assert!(upper.path().join(".wh.hello.txt").exists());
    // Lower file still exists.
    assert!(dir.path().join("hello.txt").exists());
}

#[test]
fn ovl_dir_iterdir_merges() {
    let dir = create_test_dir();
    let upper = TempDir::new().unwrap();
    let mut mt = mount_at_mnt(
        &dir,
    ...
    call_write(
        ...
        OsFunction::WriteText,
        "/mnt/upper_only.txt",
        MontyObject::String("upper".to_owned()),
    )
    .unwrap()
    .unwrap();
    ...
    let result = call_ok(&mut mt, OsFunction::Iterdir, "/mnt");
    let names = sorted_names(&result);
    assert!(names.contains(&"hello.txt".to_owned()), "lower file present");
    assert!(names.contains(&"upper_only.txt".to_owned()), "upper file present");
}

#[test]
fn ovl_dir_iterdir_respects_whiteouts() {
    let dir = create_test_dir();
    let upper = TempDir::new().unwrap();
    let mut mt = mount_at_mnt(
        &dir,
    ...
    let result = call_ok(&mut mt, OsFunction::Iterdir, "/mnt");
    let names = sorted_names(&result);
    assert!(!names.contains(&"hello.txt".to_owned()), "whited out file hidden");
    // Whiteout files themselves should not appear.
    assert!(
        !names.contains(&".wh.hello.txt".to_owned()),
        "whiteout marker should not appear"
    );
}

#[test]
fn ovl_dir_upper_priority_on_read() {
    let dir = create_test_dir();
    let upper = TempDir::new().unwrap();
    let mut mt = mount_at_mnt(
        &dir,
    ...
    // Overwrite a lower file via overlay.
    call_write(
        &mut mt,
        OsFunction::WriteText,
        "/mnt/hello.txt",
        MontyObject::String("from upper".to_owned()),
    )
    .unwrap()
    .unwrap();
...
fn ovl_dir_mkdir() {
    ...
    call_mkdir(&mut mt, "/mnt/new_upper_dir", false, false)
        .unwrap()
        .unwrap();
    assert_eq!(
        call_ok(&mut mt, OsFunction::IsDir, "/mnt/new_upper_dir"),
        MontyObject::Bool(true)
    );
    assert!(upper.path().join("new_upper_dir").is_dir());
}
