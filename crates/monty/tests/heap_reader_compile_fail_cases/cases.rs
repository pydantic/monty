...
fn smuggle_vm(
    heap: &mut Heap,
    tables: &'static mut crate::run::SessionTables,
    program: &'static crate::run::Program,
) -> VM<'static> {
    HeapReader::with(
        heap,
        &mut (tables, monty_types::PrintWriter::Disabled),
        |reader, (tables, print)| VM::new(Vec::new(), tables, program, reader, print.reborrow()),
    )
}
