...
fn repl_functools_callables_cross_as_their_repr() {
    ...
    assert_eq!(
        ...
        MontyObject::repr("functools.partial(<built-in function len>, [1])")
    );
    let wrapper = feed_run_print(&mut repl, "f").unwrap();
    let MontyNode::Repr(cached) = unstable::root_node(&wrapper) else {
