...
with Tracked('a') as bound:
    log.append('body')
    assert bound.name == 'a', 'as-target gets the __enter__ return value'
assert log == ['enter a', 'body', 'exit a'], 'enter/body/exit ordering'
...
with EnterValue() as v:
    assert v == 42, 'as-target gets non-self __enter__ return value'
...
with g:
    pass
assert g.typ is None, 'normal exit: typ is None'
assert g.val is None, 'normal exit: val is None'
assert g.tb is None, 'normal exit: tb is None'
...
assert g.typ is ValueError, 'exception path: typ is the exception type'
assert isinstance(g.val, ValueError), 'exception path: val is the exception instance'
assert str(g.val) == 'boom', 'exception path: val carries the message'
...
swallowed = True
assert swallowed, 'truthy __exit__ lets control fall through to after the with'
...
try:
    ...
    caught = str(e)
assert caught == 'passthrough-prop', 'None-returning __exit__ does not suppress'

# The return value is ignored entirely on the normal-exit path.
with Grab():
    inside = 'ran'
assert inside == 'ran', 'normal-exit path ignores the __exit__ return value'
...
assert caught_info == ('ValueError', 'cleanup-failed'), '__exit__ exception replaces in-flight RuntimeError'
...
assert caught == "'NoEnter' object does not support the context manager protocol (missed __enter__ method)", (
    'class with __exit__ but no __enter__'
)
...
assert caught == "'NoExit' object does not support the context manager protocol (missed __exit__ method)", (
    '__exit__ is checked before __enter__'
)
...
assert caught == "'Neither' object does not support the context manager protocol (missed __exit__ method)", (
    'class with neither dunder reports __exit__'
)
...
try:
    ...
    caught = str(e)
assert caught == "'int' object is not callable", 'non-callable __enter__ raises when called'
...
with DynCm() as v:
    assert v == 'dyn', 'dynamic type() classes with lambda dunders support the protocol'

# === Direct __enter__() / __exit__() calls are ordinary method calls ===
p = Passthrough()
assert p.__enter__() is p, 'direct __enter__() works and returns self'
assert p.__exit__(None, None, None) is None, 'direct __exit__() returns None'
assert Grab().__exit__(ValueError, ValueError('x'), None) is True, 'direct __exit__ args reach the method verbatim'
...
with s as v:
    assert v == 'class-enter', 'with-statement lookup skips the instance __dict__'
assert s.__enter__() == 'instance-enter', 'explicit call reads the instance attribute'
...
assert caught == 'cleanup-failed', '__exit__ runs when unpacking the as-target fails'
...
assert early_return() == 'early', 'return inside with returns its value'
assert log == ['enter ret', 'exit ret'], 'return inside with calls __exit__'
...
assert log == [
    ...
    'enter loop2',
    'exit loop2',
], 'break/continue inside with call __exit__'

# === Nested and multi-item with: LIFO exit order ===
log = []
with Tracked('outer'), Tracked('inner'):
    log.append('body')
assert log == ['enter outer', 'enter inner', 'body', 'exit inner', 'exit outer'], 'multi-item with exits in LIFO order'
...
with Passthrough():
    try:
        ...
        outcome = 'propagated'
assert outcome == 'fell-through', 'inner suppress swallows before the outer with is involved'
...
try:
    ...
    caught_info = ('RuntimeError', str(e))
assert caught_info == ('ValueError', 'cleanup-failed'), 'outer __exit__ raising replaces the inner exception'
