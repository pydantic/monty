...
impl HeapData {
    ...
    fn compute_hash_if_immutable(&self, heap: &mut Heap<impl ResourceTracker>, interns: &Interns) -> Option<u64> {
        match self {
            ...
            // Mutable types, exceptions, iterators, modules, regex types, and async types cannot be hashed
...
impl PyTrait for HeapData {
    ...
    fn py_dec_ref_ids(&mut self, stack: &mut Vec<HeapId>) {
        match self {
            ...
            // Range, Slice, Exception, LongInt, Path, RePattern, and ReMatch have no nested heap references
            Self::Range(_)
            | Self::Slice(_)
            | Self::Exception(_)
            | Self::LongInt(_)
...
impl HashState {
    fn for_data(data: &HeapData) -> Self {
        match data {
            ...
            // Mutable containers, exceptions, iterators, modules, regex types, and async types are unhashable
            HeapData::List(_)
            | HeapData::Dict(_)
            | HeapData::Set(_)
            ...
            | HeapData::ReMatch(_) => Self::Unhashable,
        }
    }
}
