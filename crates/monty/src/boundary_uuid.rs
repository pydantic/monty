//! Worker-side uuid minting for sandbox-defined classes and instances.
//!
//! Sandbox objects need boundary identities minted where they live — in the
//! worker — so ids never encode addresses and stay stable for the life of the
//! heap object (dumps included). Minting only happens when a value crosses to
//! the host; sandboxed code has no way to observe these ids, so this is not
//! an entropy side-channel into the sandbox.

use monty_types::MontyUuid;

/// Mints a fresh uuid4 from OS entropy.
///
/// # Panics
