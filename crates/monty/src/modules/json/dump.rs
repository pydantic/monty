...
///
/// Arity and missing-arg errors use the `dumps()` descriptor with `py_def`
/// wording (CPython's `json.dumps` is a pure-Python `def`), but the
...
#[derive(FromArgs)]
#[from_args(name = "dumps", py_def, kwarg_error_name = "JSONEncoder.__init__")]
