...
//!
//! Parsing uses `jiter` so we get JSON-specific syntax errors and big-integer
//! support without routing through `serde_json`. Serialization writes directly
//! into a `String` buffer to avoid constructing an intermediate JSON AST.
...
use crate::{
    ...
    types::{Dict, List, LongInt, Module, PyTrait, long_int::check_bits_str_digits_limit, str::allocate_string},
    value::Value,
};
...
pub(super) fn call(
    ...
    match function {
        JsonFunctions::Loads => call_loads(vm, args),
        JsonFunctions::Dumps => call_dumps(vm, args),
    }
}
...
fn call_loads(vm: &mut VM<'_, '_, impl ResourceTracker>, args: ArgValues) -> RunResult<Value> {
...
fn call_dumps(vm: &mut VM<'_, '_, impl ResourceTracker>, args: ArgValues) -> RunResult<Value> {
    let (mut pos, kwargs) = args.into_parts();
