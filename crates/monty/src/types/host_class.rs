...
/// resolves to that object instead (see `object_bridge`). Holds the class
/// name, the host-generated boundary identities, and the eagerly sent
/// attributes. Names missing from `attrs` route back to the host:
...
/// when a host passes a bare class in (a `Type` input whose id names no live
/// sandbox class; one that does resolves to that class object instead).
///
...
// Custom serde implementation for HostClass; serializes all five fields so
// suspended state (dumps) round-trips exactly.
impl serde::Serialize for HostClass {
    fn serialize<S: serde::Serializer>(&self, serializer: S) -> Result<S::Ok, S::Error> {
        let mut state = serializer.serialize_struct("HostClass", 5)?;
