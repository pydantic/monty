    ...
    fn instantiate_class(&mut self, class_id: HeapId, args: ArgValues) -> Result<CallResult, RunError> {
        ...
        match init {
            None => {
                ...
                // natively off the recorded metadata (no generated bytecode);
                // `modules::dataclasses` owns that binding.
                if dataclasses::is_dataclass_class(class_id, self) {
                    dataclasses::dataclass_init(self, class_id, instance_id, args)
                } else if matches!(args, ArgValues::Empty) {
