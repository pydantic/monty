...
impl VMSnapshot {
    ...
    /// stacks, frames' pending cache stores, scheduler tasks, pending resume
    /// effects), and returns the globals, working directory and `random`
    /// generator so an abandoned REPL snippet keeps its namespace, any
...
impl<'h> VM<'h> {
    ...
    pub fn snapshot(mut self) -> VMSnapshot {
        ...
        let frames_snapshot = if self.current_frame.is_parked {
            ...
            let current = mem::replace(&mut self.current_frame, CallFrame::new_parked(code));
            suspended
                .into_iter()
                .chain([current])
                .map(CallFrame::into_serialized)
                .collect()
        };
