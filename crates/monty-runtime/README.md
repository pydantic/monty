...
an embedder chooses otherwise; the CLI has no flag for it. The CLI has no
`os` handler, so `os.urandom()` and any unseeded `random` draw raise
`RuntimeError`. Seed the generator first.
