...
fn large_allocations_are_rejected_before_the_hard_limit() {
    ...
    let cases = [
        ("'x' * 10_000_000", 10_042_058),
        // Each formatter builder must fail softly before the worker reaches its hard ceiling.
        ("s = 'x' * 400_000\n'{0}{0}'.format(s)", 1_242_489),
        ("s = 'x' * 400_000\n'{0:>1000000}'.format(s)", 1_442_521),
        ("s = 'é' * 200_000\n'{0!a}'.format(s)", 1_242_526),
        // `%` formatting: padding, float digits, integer zero-extension and output growth.
        ("'%*d' % (2_000_000, 1)", 2_042_215),
        ("'%.*f' % (1_000_000, 1.0)", 1_171_255),
        ("'%.*d' % (2_000_000, 1)", 2_042_219),
        ("s = 'x' * 400_000\n'%s%s' % (s, s)", 1_642_675),
        ("b'%*d' % (2_000_000, 1)", 2_050_437),
        ("s = b'x' * 400_000\nb'%s%s' % (s, s)", 1_650_898),
        ("b'x' * 10_000_000", 10_050_292),
        ("[None] * 1_000_000", 16_042_213),
        ("2 ** 10_000_000", 10_042_057),
        ("1 << 10_000_000", 1_292_058),
        ...
        ("import math\nmath.factorial(2_000_000)", 10_547_220),
        // A binomial is bounded by `2**n`, so `comb` needs a larger `n` to trip the check.
        ("import math\nmath.comb(9_000_000, 4_500_000)", 2_297_237),
        ("import math\nmath.perm(4_000_000, 2_000_000)", 11_047_237),
        // `math.lcm` of two large coprime ints is a product, preflighted like `*`.
        ("import math\nx = 1 << 2_000_000\nmath.lcm(x + 1, x - 1)", 1_297_567),
        ("('a' * 1000).replace('a', 'b' * 2000)", 2_045_550),
        ...
        ("import random\nrandom.seed(0)\nrandom.randbytes(600_000)", 1_247_295),
