...
WebAssembly build of the Monty sandboxed Python interpreter, for browsers and
other environments where spawning subprocesses is impossible. On Node.js,
prefer [`@pydantic/monty`](https://www.npmjs.com/package/@pydantic/monty),
which runs the interpreter in crash-isolated worker subprocesses.

## Installation

```bash
npm install @pydantic/monty-wasm
```
...
```ts
import { Monty, runMontyAsync } from '@pydantic/monty-wasm'
...
```ts
import { Monty, MontySyntaxError, MontyRuntimeError, MontyTypingError } from '@pydantic/monty-wasm'
