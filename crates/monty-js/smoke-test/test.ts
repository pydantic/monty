...
export interface SmokeTestOptions {
  expectWorkerPid: boolean
}

/** Exercises the public Monty API shared by the released Node and browser packages. */
export async function runSmokeTest({ expectWorkerPid }: SmokeTestOptions): Promise<void> {
  let passed = 0
  let failed = 0
  ...
  async function assertThrowsAsync<T extends Error>(
    ...
    try {
      ...
      console.error(`FAIL: ${message} - wrong error type: ${unexpected.constructor.name}: ${unexpected.message}`)
      failed++
      return null
    }
  }
  ...
  assert(
    expectWorkerPid ? typeof session.workerPid === 'number' : session.workerPid == null,
    expectWorkerPid ? 'Node session has a worker pid' : 'browser session has no process id',
  )
  ...
  assert(dumped instanceof Uint8Array, 'dump returns bytes')
  ...
  if (failed > 0) {
    throw new Error(`${failed} smoke test${failed === 1 ? '' : 's'} failed`)
  }
