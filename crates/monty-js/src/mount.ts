...
import { MountMode, MountSchema, type Mount } from './generated/monty/v1/monty_pb.js'
...
export interface MountDirOptions {
   ...
   * worker-local memory and discards them when the session ends.
   */
  mode?: MountDirMode
...
const MODE_TO_PROTO: Record<MountDirMode, MountMode> = {
  'read-only': MountMode.READ_ONLY,
  'read-write': MountMode.READ_WRITE,
...
/** Encodes the `mount` option (one or many) as wire `Mount`s. */
export function mountsToProto(mount: MountDir | MountDir[] | undefined): Mount[] {
  if (mount === undefined) {
    return []
  }
  ...
  return mounts.map((m) =>
    create(MountSchema, {
      ...
      hostPath: m.hostPath,
      mode: MODE_TO_PROTO[m.mode],
      ...(m.writeBytesLimit !== null ? { writeBytesLimit: BigInt(m.writeBytesLimit) } : {}),
    }),
  )
}
