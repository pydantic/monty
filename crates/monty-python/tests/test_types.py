...
def test_none_input():
    ...
    assert m.run(inputs={'x': None}).output is True


def test_none_output():
    m = pydantic_monty.Monty('None')
    assert m.run().output is None


def test_bool_true():
    m = pydantic_monty.Monty('x', inputs=['x'])
    result = m.run(inputs={'x': True}).output
...
def test_bool_false():
    m = pydantic_monty.Monty('x', inputs=['x'])
    result = m.run(inputs={'x': False}).output
...
def test_int():
    ...
    assert m.run(inputs={'x': -100}).output == snapshot(-100)
    assert m.run(inputs={'x': 0}).output == snapshot(0)


def test_float():
    m = pydantic_monty.Monty('x', inputs=['x'])
    assert m.run(inputs={'x': 3.14}).output == snapshot(3.14)
    assert m.run(inputs={'x': -2.5}).output == snapshot(-2.5)
    assert m.run(inputs={'x': 0.0}).output == snapshot(0.0)


def test_string():
    m = pydantic_monty.Monty('x', inputs=['x'])
    assert m.run(inputs={'x': 'hello'}).output == snapshot('hello')
    assert m.run(inputs={'x': ''}).output == snapshot('')
    assert m.run(inputs={'x': 'unicode: éè'}).output == snapshot('unicode: éè')


def test_bytes():
    m = pydantic_monty.Monty('x', inputs=['x'])
    assert m.run(inputs={'x': b'hello'}).output == snapshot(b'hello')
    assert m.run(inputs={'x': b''}).output == snapshot(b'')
    assert m.run(inputs={'x': b'\x00\x01\x02'}).output == snapshot(b'\x00\x01\x02')


def test_list():
    m = pydantic_monty.Monty('x', inputs=['x'])
    assert m.run(inputs={'x': [1, 2, 3]}).output == snapshot([1, 2, 3])
    assert m.run(inputs={'x': []}).output == snapshot([])
    assert m.run(inputs={'x': ['a', 'b']}).output == snapshot(['a', 'b'])
...
def test_tuple():
    ...
    assert m.run(inputs={'x': (1, 2, 3)}).output == snapshot((1, 2, 3))
    assert m.run(inputs={'x': ()}).output == snapshot(())
    assert m.run(inputs={'x': ('a',)}).output == snapshot(('a',))


def test_dict():
    m = pydantic_monty.Monty('x', inputs=['x'])
    assert m.run(inputs={'x': {'a': 1, 'b': 2}}).output == snapshot({'a': 1, 'b': 2})
    assert m.run(inputs={'x': {}}).output == snapshot({})
...
def test_set():
    m = pydantic_monty.Monty('x', inputs=['x'])
    assert m.run(inputs={'x': {1, 2, 3}}).output == snapshot({1, 2, 3})
    assert m.run(inputs={'x': set()}).output == snapshot(set())


def test_frozenset():
    m = pydantic_monty.Monty('x', inputs=['x'])
    assert m.run(inputs={'x': frozenset([1, 2, 3])}).output == snapshot(frozenset({1, 2, 3}))
    assert m.run(inputs={'x': frozenset()}).output == snapshot(frozenset())


def test_ellipsis_input():
    m = pydantic_monty.Monty('x is ...', inputs=['x'])
    assert m.run(inputs={'x': ...}).output is True


def test_ellipsis_output():
    m = pydantic_monty.Monty('...')
    assert m.run().output is ...


def test_nested_list():
    m = pydantic_monty.Monty('x', inputs=['x'])
    nested = [[1, 2], [3, [4, 5]]]
    assert m.run(inputs={'x': nested}).output == snapshot([[1, 2], [3, [4, 5]]])


def test_nested_dict():
    m = pydantic_monty.Monty('x', inputs=['x'])
    nested = {'a': {'b': {'c': 1}}}
    assert m.run(inputs={'x': nested}).output == snapshot({'a': {'b': {'c': 1}}})


def test_mixed_nested():
    m = pydantic_monty.Monty('x', inputs=['x'])
    mixed = {'list': [1, 2], 'tuple': (3, 4), 'nested': {'set': {5, 6}}}
    result = m.run(inputs={'x': mixed}).output
...
def test_list_output():
    ...
    assert m.run().output == snapshot([1, 2, 3])


def test_dict_output():
    m = pydantic_monty.Monty("{'a': 1, 'b': 2}")
    assert m.run().output == snapshot({'a': 1, 'b': 2})
...
def test_date_input_roundtrip():
    m = pydantic_monty.Monty('x', inputs=['x'])
    result = m.run(inputs={'x': datetime.date(2024, 1, 15)}).output
    assert (type(result).__name__, repr(result)) == snapshot(('date', 'datetime.date(2024, 1, 15)'))


def test_datetime_input_roundtrip():
    m = pydantic_monty.Monty('x', inputs=['x'])
    result = m.run(inputs={'x': datetime.datetime(2024, 1, 15, 10, 30, 5, 123456)}).output
...
def test_aware_datetime_input_roundtrip():
    m = pydantic_monty.Monty('x', inputs=['x'])
    result = m.run(inputs={'x': datetime.datetime(2024, 1, 15, 10, 30, 5, 123456, tzinfo=datetime.timezone.utc)}).output
    assert (type(result).__name__, repr(result)) == snapshot(
...
def test_timedelta_input_roundtrip():
    m = pydantic_monty.Monty('x', inputs=['x'])
    result = m.run(inputs={'x': datetime.timedelta(days=-1, seconds=3661, microseconds=42)}).output
    assert (type(result).__name__, repr(result)) == snapshot(
...
def test_timezone_input_roundtrip():
    m = pydantic_monty.Monty('x', inputs=['x'])
    result = m.run(inputs={'x': datetime.timezone(datetime.timedelta(hours=2))}).output
    assert (type(result).__name__, repr(result)) == snapshot(
...
def test_named_timezone_input_roundtrip():
    m = pydantic_monty.Monty('x', inputs=['x'])
    result = m.run(inputs={'x': datetime.timezone(datetime.timedelta(hours=2), 'PLUS2')}).output
    assert (type(result).__name__, repr(result)) == snapshot(
...
def test_exception_input():
    ...
    result = m.run(inputs={'x': exc}).output
...
def test_int_subclass_input():
    ...
    m = pydantic_monty.Monty('x', inputs=['x'])
    result = m.run(inputs={'x': MyInt(42)}).output
...
def test_str_subclass_input():
    ...
    m = pydantic_monty.Monty('x', inputs=['x'])
    result = m.run(inputs={'x': MyStr('hello')}).output
...
def test_list_subclass_input():
    ...
    m = pydantic_monty.Monty('x', inputs=['x'])
    result = m.run(inputs={'x': MyList([1, 2, 3])}).output
...
def test_dict_subclass_input():
    ...
    m = pydantic_monty.Monty('x', inputs=['x'])
    result = m.run(inputs={'x': MyDict({'a': 1})}).output
...
def test_tuple_subclass_input():
    ...
    m = pydantic_monty.Monty('x', inputs=['x'])
    result = m.run(inputs={'x': MyTuple((1, 2))}).output
...
def test_set_subclass_input():
    ...
    m = pydantic_monty.Monty('x', inputs=['x'])
    result = m.run(inputs={'x': MySet({1, 2})}).output
...
def test_bigint_input():
    ...
    big = 2**100
    m = pydantic_monty.Monty('x', inputs=['x'])
    result = m.run(inputs={'x': big}).output
    assert result == big
    assert type(result) is int
...
def test_bigint_negative_input():
    ...
    big_neg = -(2**100)
    m = pydantic_monty.Monty('x', inputs=['x'])
    result = m.run(inputs={'x': big_neg}).output
...
def test_int_overflow_to_bigint():
    ...
    result = m.run(inputs={'x': max_i64}).output
...
def test_bigint_arithmetic():
    ...
    result = m.run(inputs={'x': big, 'y': big}).output
...
def test_bigint_comparison():
    ...
    assert m.run(inputs={'x': big, 'y': 42}).output is True
    assert m.run(inputs={'x': 42, 'y': big}).output is False
...
def test_bigint_in_collection():
    ...
    big = 2**100
    m = pydantic_monty.Monty('x', inputs=['x'])
    result = m.run(inputs={'x': [big, 42, big * 2]}).output
...
def test_bigint_as_dict_key():
    ...
    big = 2**100
    m = pydantic_monty.Monty('x', inputs=['x'])
    result = m.run(inputs={'x': {big: 'value'}}).output
...
def test_bigint_hash_consistency_small_values():
    ...
    computed_42 = m.run(inputs={'x': big}).output
...
def test_bigint_hash_consistency_boundary():
    ...
    m = pydantic_monty.Monty('(x - 1)', inputs=['x'])
    computed_max = m.run(inputs={'x': max_i64 + 1}).output
...
def test_bigint_hash_consistency_large_values():
    ...
    m = pydantic_monty.Monty('x', inputs=['x'])
    result1 = m.run(inputs={'x': big1}).output
    result2 = m.run(inputs={'x': big2}).output
...
def test_namedtuple_custom_input_attribute_access():
    ...
    assert m.run(inputs={'p': Person(name='Alice', age=30)}).output == snapshot('Alice')

    m = pydantic_monty.Monty('p.age', inputs=['p'])
    assert m.run(inputs={'p': Person(name='Alice', age=30)}).output == snapshot(30)
...
def test_namedtuple_custom_input_index_access():
    ...
    assert m.run(inputs={'p': Point(x=10, y=20)}).output == snapshot(30)
...
def test_namedtuple_custom_input_multiple_fields():
    ...
    result = m.run(inputs={'c': Config(host='localhost', port=8080, debug=True, timeout=30.0)}).output
...
def test_namedtuple_custom_input_repr():
    ...
    result = m.run(inputs={'item': Item(name='widget', price=9.99)}).output
...
def test_namedtuple_custom_input_len():
    ...
    assert m.run(inputs={'t': Triple(a=1, b=2, c=3)}).output == snapshot(3)
...
def test_namedtuple_custom_input_roundtrip():
    ...
    result = m.run(inputs={'p': Pair(first=1, second=2)}).output
...
def test_callable_input():
    ...
    m = pydantic_monty.Monty('x', inputs=['x'])
    result = m.run(inputs={'x': len}).output
...
def test_lambda_input():
    ...
    m = pydantic_monty.Monty('x', inputs=['x'])
    result = m.run(inputs={'x': lambda: None}).output
...
def test_utc_timezone_input_roundtrip():
    ...
    m = pydantic_monty.Monty('x', inputs=['x'])
    result = m.run(inputs={'x': datetime.timezone.utc}).output
...
def test_negative_timezone_offset():
    ...
    result = m.run(inputs={'x': tz}).output
...
def test_aware_datetime_fixed_offset_roundtrip():
    ...
    result = m.run(inputs={'x': dt}).output
    assert (type(result).__name__, repr(result)) == snapshot(
        (
            'datetime',
...
def test_timedelta_zero():
    ...
    m = pydantic_monty.Monty('x', inputs=['x'])
    result = m.run(inputs={'x': datetime.timedelta(0)}).output
...
def test_timedelta_days_only():
    ...
    m = pydantic_monty.Monty('x', inputs=['x'])
    result = m.run(inputs={'x': datetime.timedelta(days=30)}).output
...
def test_path_input_roundtrip():
    ...
    m = pydantic_monty.Monty('x', inputs=['x'])
    result = m.run(inputs={'x': pathlib.PurePosixPath('/usr/local/bin')}).output
...
def test_posix_path_input():
    ...
    m = pydantic_monty.Monty('x', inputs=['x'])
    result = m.run(inputs={'x': pathlib.PosixPath('/tmp')}).output
...
def test_float_subclass_input():
    ...
    m = pydantic_monty.Monty('x', inputs=['x'])
    result = m.run(inputs={'x': MyFloat(3.14)}).output
...
def test_bytes_subclass_input():
    ...
    m = pydantic_monty.Monty('x', inputs=['x'])
    result = m.run(inputs={'x': MyBytes(b'hello')}).output
...
def test_frozenset_subclass_input():
    ...
    m = pydantic_monty.Monty('x', inputs=['x'])
    result = m.run(inputs={'x': MyFrozenSet([1, 2, 3])}).output
