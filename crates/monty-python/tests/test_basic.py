...
def test_simple_expression():
    ...
    assert m.run().output == snapshot(3)


def test_arithmetic():
    m = pydantic_monty.Monty('10 * 5 - 3')
    assert m.run().output == snapshot(47)


def test_string_concatenation():
    m = pydantic_monty.Monty('"hello" + " " + "world"')
    assert m.run().output == snapshot('hello world')


def test_multiple_runs_same_instance():
    m = pydantic_monty.Monty('x * 2', inputs=['x'])
    assert m.run(inputs={'x': 5}).output == snapshot(10)
    assert m.run(inputs={'x': 10}).output == snapshot(20)
    assert m.run(inputs={'x': -3}).output == snapshot(-6)
...
"""
    m = pydantic_monty.Monty(code)
    assert m.run().output == snapshot(7)
