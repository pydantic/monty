...
mod tests {
    ...
    use monty_types::{CallArgs, MontyObject, MontyValue, NameLookupResult};
    ...
    fn one_feed_produces_a_nested_span_tree() {
        ...
        recorder.begin_turn(&request(pb::parent_request::Kind::ResumeCall(pb::ResumeCall {
            call_id: 1,
            ..pb::ResumeCall::from(MontyValue::from(MontyObject::Int(4)))
        })));
        recorder.event(&event(pb::child_event::Kind::Complete(pb::Complete::from(
            MontyValue::from(MontyObject::Int(4)),
        ))));
    ...
    fn suspension_answers_land_on_their_span() {
        ...
        recorder.begin_turn(&request(pb::parent_request::Kind::ResumeCall(pb::ResumeCall {
            call_id: 1,
            ..pb::ResumeCall::from(MontyValue::from(MontyObject::String(long)))
        })));
    ...
    fn idle_restore_keeps_its_session_parent() {
        ...
        recorder.event(&event(pb::child_event::Kind::Complete(pb::Complete::from(
            MontyValue::from(MontyObject::Int(1)),
        ))));
        recorder.begin_turn(&request(pb::parent_request::Kind::Reset(pb::Reset {})));
    ...
    fn completion_without_a_feed_span_is_recorded() {
        ...
        let mut recorder = Recorder::new(None);
        recorder.event(&event(pb::child_event::Kind::Complete(pb::Complete::from(
            MontyValue::from(MontyObject::Int(7)),
        ))));
