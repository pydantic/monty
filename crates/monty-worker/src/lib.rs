...
use monty_proto::{
    FrameError, FrameReader, MAX_FRAME_LEN, WireFunctionCall, WireOsCall, build_mount_table, exceeds_max_value_depth,
...
impl Child {
    ...
    pub fn handle(
        ...
        let mut event = match kind {
            ...
            pb::parent_request::Kind::ReplFeed(feed) => self.handle_repl_feed(feed, sink),
    ...
    fn handle_repl_create(&mut self, create: pb::ReplCreate) -> pb::ChildEvent {
        if !matches!(self.state, SessionState::Idle) {
            return protocol_violation("ReplCreate while a session already exists");
        }
    ...
    fn handle_repl_feed(&mut self, feed: pb::ReplFeed, sink: &mut dyn EventSink) -> pb::ChildEvent {
        let SessionState::Ready(_) = &self.state else {
            return protocol_violation("ReplFeed without a session ready for input");
        };
    ...
    fn handle_dump(&mut self) -> pb::ChildEvent {
        let dumped = match &self.state {
            ...
            SessionState::Idle => return protocol_violation("Dump without a session"),
        };
        match dumped {
            Ok((tag, payload)) => {
    ...
    fn handle_load(&mut self, load: &pb::Load) -> pb::ChildEvent {
        if !matches!(self.state, SessionState::Idle) {
            return protocol_violation("Load while a session already exists");
        }
