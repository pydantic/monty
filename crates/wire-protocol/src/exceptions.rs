//! Conversions between Python exception objects and monty's exception types.
//!
//! This is the exception half of the value-conversion layer: it maps a
//! `MontyException` onto the matching *native* Python exception (`ValueError`,
//! `json.JSONDecodeError`, ...) and back, so an exception can cross the sandbox
//! boundary in either direction and surface as the type a Python user expects.
//!
//! These functions deal only in builtin / stdlib exception types — they never
//! construct the `pydantic_monty.MontyError` wrapper hierarchy, which lives in
//! the `pydantic_monty` package and is layered on top of `exc_monty_to_py`.

use ::monty::{ExcType, MontyException};
use pyo3::{PyTypeCheck, exceptions, prelude::*, sync::PyOnceLock, types::PyString};
